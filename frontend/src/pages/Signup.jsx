...
export default function Signup() {
  ...
  const validate = () => {
    ...
    if (!form.name.match(/^[A-Za-z ]+$/)) {
      newErr.name = "Name can only contain letters and spaces.";
    }
      ...
      <Card className="relative z-10 w-[380px] bg-[#0d1522]/70 backdrop-blur-xl border border-white/10 shadow-[0_0_40px_rgba(0,255,120,0.06)] rounded-2xl">
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {/* NAME */}
            <div>
              <Label className="text-gray-200 pb-2">Name</Label>
              <Input
                name="name"
                value={form.name}
              ...
              />
              {err.name && <p className="text-red-400 text-sm">{err.name}</p>}
            </div>
            ...
            <div>
              ...
              />
              {err.password && (
                <p className="text-red-400 text-sm">{err.password}</p>
              )}
            </div>

            {/* ROLE */}
            <div>
              <Label className="text-gray-200 pb-2">Role</Label>
              <select
                name="role"
                value={form.role}
                onChange={handleChange}
